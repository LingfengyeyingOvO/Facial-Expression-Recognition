...
callbacks = [
    ...
    ModelCheckpoint('best_dropout_model.h5', save_best_only=True)
]
...
df.to_csv('training_dropout_history.csv', index=False)
print("训练历史已保存到 training_dropout_history.csv")
...
# 保存最终模型（可选）
emotion_model.save('emotion_dropout_model.h5')
print("模型已保存到 emotion_dropout_model.h5")
...
# 也可以保存到文件
plt.savefig('training_dropout_curves.png')
print("训练曲线已保存为 training_dropout_curves.png")